# Global constants
PATH_TO_CHROME_WEBDRIVER = "P:/Pycharm/PycharmProjects/DemoWebShop/drivers/chromedriver.exe"
PATH_TO_FIREFOX_WEBDRIVER = "P:/Pycharm/PycharmProjects/DemoWebShop/drivers/geckodriver.exe"
PATH_TO_USERS_JSON = "P:/Pycharm/PycharmProjects/DemoWebShop/constants/credentials.json"
START_PAGE_url = "http://demowebshop.tricentis.com/"
REGISTER_PAGE_url = f"{START_PAGE_url}register"
LOGIN_PAGE_url = f"{START_PAGE_url}login"

CHROME = "chrome"
FIREFOX = "firefox"
