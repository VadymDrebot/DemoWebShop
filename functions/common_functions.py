...
class CommonFunctions(Waitings):
    ...
    def clear_input_field(self, locator):
        input_field = self.driver.find_element_by_xpath(locator)
        input_field.clear()
    ...
    @wait_5_sec
    def click_button_and_verify_message_by_xpath(self, button_locator, xpath_expecting_locator, expected_message):
        self.wait_click_ability_and_click(locator=button_locator)
        self.wait_until_visibility_of_element(locator=xpath_expecting_locator, period=2)
        actual_message = self.driver.find_element_by_xpath(xpath_expecting_locator).text
        # self.logger.info(f"expected message: {expected_message} , actual message: {actual_message}")
        assert actual_message == expected_message

    @wait_5_sec
    def click_button_and_verify_xpath(self, button_locator, xpath_expecting_locator):
        # Click  button(by locator) and check XPATH(expecting_locator) for approving exist element
        self.wait_click_ability_and_click(locator=button_locator)
        self.wait_until_visibility_of_element(locator=xpath_expecting_locator, period=2)
        assert self.driver.find_element_by_xpath(xpath_expecting_locator)
    ...
    def choose_random_from_drop_list(self, locator):
    ...
    # @log
    def verify_message(self, locator, expected_text):
    ...
    def verify_text_presents_in_message(self, message_locator, expected_text):
        # verify that expected_text IN the locator's message
        message = self.get_text_from_xpath(message_locator)
    ...
    def get_value_from_input_field(self, locator):
        ...
        return self.driver.find_element_by_xpath(locator).get_attribute('value')

    def get_text_from_xpath(self, locator):
        if self.verify_presence_of_element(locator):
            # WebDriverWait(self.driver, timeout=5).until(EC.presence_of_element_located((By.XPATH, locator)))
            textt=self.driver.find_element_by_xpath(locator).text
            return textt
        else:
            return "empty field"
